/** Verde de farmacia no Locus — reutilizado para clinicas veterinarias. */
...
export function buildVetClinicPinHtml() {
  return `<div class="lyka-poi-pin-balloon" aria-hidden="true"><div class="lyka-poi-pin lyka-poi-pin--veterinary" style="--poi-color:${VET_CLINIC_POI_COLOR}">${VET_CLINIC_POI_SVG}</div></div>`;
}

export function buildVetClinicPopupHtml(clinic: { title: string; address: string }) {
  const title = escapeHtml(clinic.title);
  const address = clinic.address.trim() ? `<div class="lyka-poi-popup-address">${escapeHtml(clinic.address)}</div>` : "";
  return `<div class="lyka-poi-popup"><div class="lyka-poi-popup-title">${title}</div>${address}</div>`;
}
